...
int PCA9685::check_pwm_ms(const int channel, const double ms)
{
    auto period_ms = 1000.0 / frequency;
    auto bits_per_ms = 4096 / period_ms;
    uint16_t bits = ms * bits_per_ms;
    const auto channel_offset = 4 * channel;
    if(i2c_dev->ReadRegisterByte(LED0_ON_L+channel_offset) != (0 & 0xFF))
        return -1;
    if(i2c_dev->ReadRegisterByte(LED0_ON_H+channel_offset) != (0 >> 8))
        return -1;
    if(i2c_dev->ReadRegisterByte(LED0_OFF_L+channel_offset) != (bits & 0xFF))
        return -1;
    if(i2c_dev->ReadRegisterByte(LED0_OFF_H+channel_offset) != (bits >> 8))
        return -1;
